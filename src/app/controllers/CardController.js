...
class CardController {
  async store(req, res) {
    ...
    const schema = Yup.object().shape({
      ...
      card_number: Yup.number().min(16).required(),
    ...
    const { card_owner, card_number, due_date, client_id } = req.body;
    ...
    const { id } = await Card.create({ card_owner, card_number, due_date, client_id });
